...
{
	FIntVector Coord(
		FMath::FloorToInt((Position.X)/ GridSize), 
		FMath::FloorToInt((Position.Y)/ GridSize), 
		0);
...
void ASquareGridManager::CreateGrids(const FIntVector& Coord, FSquareGridArray& GridArray)
{
	...
	FVector Center(
		Coord.X * GridSize + GridSize / 2.f, 
		Coord.Y * GridSize + GridSize / 2.f, 
		0.f);
...
void ASquareGridManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	/*// 打印销毁原因
	FString Reason;
	switch (EndPlayReason)
	{
	case EEndPlayReason::Destroyed: Reason = TEXT("被代码调用 Destroy() 销毁"); break;
	case EEndPlayReason::LevelTransition: Reason = TEXT("关卡切换"); break;
	case EEndPlayReason::EndPlayInEditor: Reason = TEXT("编辑器停止运行"); break;
	case EEndPlayReason::RemovedFromWorld: Reason = TEXT("从世界中移除"); break;
	case EEndPlayReason::Quit: Reason = TEXT("退出游戏"); break;
	default: Reason = TEXT("未知原因"); break;
	}

	UE_LOG(LogTemp, Error, TEXT("!!!警告!!! SquareGridManager 正在消失！原因: %s"), *Reason);*/

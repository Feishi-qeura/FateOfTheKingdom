...
public:
	...
	void FindOutlineEdges(const TArray<FGridOutlineInfo*>& GridGroup, TArray<T>& OutlineEdges)
	{
		...
		for (int i = 0; i < GridGroup.Num(); ++i)
		{
			...
			{
				GetCounterEdge((FEdgeDirType)DirIndex, Coord, CounterDir, CounterCoord);

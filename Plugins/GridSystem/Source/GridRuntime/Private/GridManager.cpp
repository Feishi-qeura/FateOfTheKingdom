...
FVector AGridManager::GetGridOrigin() const
{
	return GridOrigin;
}
void AGridManager::SetGridDataTable(UDataTable* InTileDataTable)
{
		UE_LOG(LogTemp, Warning, TEXT("GridDataTable start"));
	if (InTileDataTable == nullptr)
	{
		UE_LOG(LogTemp, Warning, TEXT("GridDataTable Empty"));
		return;
	}
	TileDataTable = InTileDataTable;
	TArray<FName> AllRowNames;
	AllRowNames = InTileDataTable->GetRowNames();
	UE_LOG(LogTemp, Warning, TEXT("SetGridDataTable Success num:%d"), AllRowNames.Num());
}
...
{
	if (AllGrids.Num() == 0 || TileIDs.Num() == 0) return;

	for (UGrid* Grid : AllGrids)
	{
		UGridInfo* Info = Cast<UGridInfo>(Grid->GridInfo);
		if (!Info) continue;

		int32 RandomIndex = FMath::RandRange(0, TileIDs.Num() - 1);
		Info->TileID = TileIDs[RandomIndex];
	}
}